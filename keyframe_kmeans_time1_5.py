...
def method2(nameVideo):
    ...
    dis = [kf_before[i] - kf_before[i - 1] for i in range(1, len(kf_before))]
    pos = np.where(np.array(dis) < 3)  # todo：帧间隔需大于2
    # print(pos[0])
    keyframe = []
    for p in pos[0]:
        keyframe.append(kf_before[p + 1])
    keyframe = list(set(kf_before) - set(keyframe))
    keyframe.sort()
    print("方法2提取关键帧数：" + str(len(keyframe)))
    return totalFrameNumber,len(keyframe)


num_mean = 0
# total_mean = 0
root = r"C:\Users\Administrator\Desktop\dataset\000151"
for video in os.listdir(root):
    print(video)
    nameVideo = os.path.join(root, video)
    total,num = method2(nameVideo)
